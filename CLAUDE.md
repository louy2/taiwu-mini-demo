...
#### 攻击判定流程 (`gameLogic.js:902-1054`)

```
1. 闪避判定 (迅疾 vs 闪避)
   闪避率 = min(50%, 闪避/(迅疾+闪避) × 50%)
   → 闪避成功: 攻击无效，流程结束
...
3. 暴击判定 (精妙 vs 拆招)
   暴击率 = min(50%, 精妙/(精妙+拆招) × 30%)
   → 暴击: 伤害 ×1.5

4. 破绽判定 (动心 vs 守心)
   破绽率 = min(30%, 动心/(动心+守心) × 20%)
   → 击中破绽: 伤害 ×1.3

5. 伤害计算 (破体/破气 vs 御体/御气)
```

#### 伤害计算 (`gameLogic.js:996-1011`)
